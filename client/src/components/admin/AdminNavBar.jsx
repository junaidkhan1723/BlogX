...
const AdminNavBar = () => {
    const {navigate, token} = useContext(AppContent)
  return (
    <>
    <div className='flex justify-between items-center mx-2 sm:mx-8'>
      ...
      <button onClick={()=> navigate('/admin')} className='flex items-center gap-1 rounded-full text-xs sm:text-sm
       cursor-pointer bg-primary hover:bg-gray-700 text-white px-2 py-2 sm:px-6 '>{token ? 'Dashboard' : 'Admin'}<img src={assets.arrow} alt="" className='w-3' /> </button>
    </div>
    </>
  )
}
